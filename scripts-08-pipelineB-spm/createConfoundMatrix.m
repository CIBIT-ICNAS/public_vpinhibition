...
confoundFile = fullfile(fmriPrepFolder,subjectID,'ses-01','func',...
    [subjectID '_ses-01_' taskName '_desc-confounds_timeseries.tsv']);
...
% Check for FSL
if isnan(nSlices)
    ME = MException('VerifyFSL', ...
        '[createConfoundMatrix] nSlices is invalid. Check if FSL is in path.');
    throw(ME);
end
...
% PhysIO
PhysioRegressors = computePhysIO(fullfile(bidsFolder,subjectID,'ses-01','func'), subjectID, taskName, nSlices, nVols, tr);
...
outputFile = fullfile(spmFolder,[subjectID '_ses-01_' taskName '_desc-PNMmodel.mat']);
