function [ OutputRunT ] = runT_MRI( run_name , S , T )
...
TestBlockDur = intervalsPRT.Coh_aCoh(1,2)-intervalsPRT.Coh_aCoh(1,1)+1;
MAEBlockDur = intervalsPRT.MAE(1,2)-intervalsPRT.MAE(1,1)+1;
...
for j = 1:size(intervalsPRT.Report,1) % Iterate on the Report blocks 
    ...
    int = intervalsPRT.Static(j,2)+2 : intervalsPRT.MAE(j,1)-2; % volumes during which the cross can increase. during the MAE, the cross does not increase
    
    testStartVol = intervalsPRT.Report(j,1) - TestBlockDur - MAEBlockDur; % test block start volume.
    ...
    if max(auxVols) < testStartVol && nEvents(j) == 4 
        ...
        fprintf('[runT] Displaced one event to Test block - volume %i.\n',testStartVol + 1);
    end
...
% Simplify nEvents to use only 2 buttons
% We did this because we will need a pulse oxymeter in one hand.
nEventsSimple = nEvents;
nEventsSimple(nEventsSimple == 2) = 1;
nEventsSimple(nEventsSimple == 3) = 2;
nEventsSimple(nEventsSimple == 4) = 2;
...
try
    ...
    while ~btUnique
        ...
        % Button 1
        DrawFormattedText(windowID, 'Press Button 1', 'center', 'center', S.white);
        Screen('Flip',windowID);
        if S.RSPBOX
            pr = 1;
            while pr
        ...
        % Button 2
        DrawFormattedText(windowID, 'Press Button 2', 'center', 'center', S.white);
        Screen('Flip',windowID);
        if S.RSPBOX
            pr = 1;
            while pr
...
%             end
            ...
            if any(framesCond(t+1) == [2,3,5,6,7,8,9,10]) || any(framesDots(t+1) == [22,33])
...
%         end % End If Not Storage
        ...
        % Fixation Cross
        drawFixationCross_MRI( windowID , S , framesCond(t+1) , framesEvents(t+1) );
        ...
        if any(t+1 == framesQ1(idxStop,:)) % Ask for Number of times
            ...
            if nEventsUser(idxStop,1) ~= 0
                if nEventsUser(idxStop,1) == nEventsSimple(idxStop) % Correct answer
                    drawFixationCross_MRI( windowID , S , 100 , framesEvents(t+1) );
                else % Incorrect answer
                    drawFixationCross_MRI( windowID , S , 101 , framesEvents(t+1) );
                end
            end
...
%         imwrite(frameImage,[pwd '\output_frames\RunB_' num2str(t+1000) '.png'],'png');
    ...
    correct_incorrect = [ sum(nEventsSimple==nEventsUser(:,1))  sum(nEventsSimple~=nEventsUser(:,1))  ];
    ...
    OutputRunT.nEventsSimple = nEventsSimple;
