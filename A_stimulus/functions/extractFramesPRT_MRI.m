function [ framesCond , framesDots , nFrames , condNames , nCond , intervalsPRT ] = extractFramesPRT_MRI( path , name , TR , fps )
...
for t = 0:nVols-1
    ...
    if any(intervalsDots(t+1) == [2,5,6,7])                    % Pattern Down
        framesDots(t*fps*TR+1:(t+1)*fps*TR) = 22;
        
    elseif any(intervalsDots(t+1) == [3,8,9,10])                % Component Out
