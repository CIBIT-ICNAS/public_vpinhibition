...
myAngle = 65;
distanceY = round(120 * T.factor);
distanceX = round(133 * T.factor);
veloc = round(2 * T.factor);
nLoopFramesY = floor(distanceY/veloc);
nLoopFramesX = floor(distanceX/veloc);
espessura = round(30 * T.factor);
...
myPhantom01 = 255 * ones(T.height , T.height , 'uint8');
...
for j = 1:nLoopFramesY % Iterate on the lines
    ...
    for i=1:size(startArrayLine,2)
        myImage02(startArrayLine(i):min(startArrayLine(i)+espessura-1,endLine),:) = 0;
    end
...
save(fullfile(S.input_path,sprintf('Textures_%i_R%i_sB%i_tB%i_l%i',T.height,S.screenX,S.screenBackground,S.textBackground,S.lines)),'T','-v7.3');
